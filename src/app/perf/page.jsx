...
export const metadata = {
  ...
  description:
    "Stack, budgets, automated test status, and Lighthouse CI thresholds enforced on every PR.",
...
export default function PerfPage() {
  return (
    <div className="relative mx-auto max-w-7xl px-4 pt-4 pb-10 space-y-4">
      ...
      <section className="card-ux">
        <h2 className="text-lg font-semibold">Lighthouse-tested on every PR</h2>
        <p className="mt-2 text-neutral-700 dark:text-neutral-300">
          Lighthouse CI runs against five routes (
          <code>/</code>, <code>/about</code>, <code>/research</code>,{" "}
          <code>/publications</code>, <code>/contact</code>) on every push to
          <code>main</code> and on every pull request. The build fails if any
          category drops below the thresholds below. Configuration lives in
          <code>lighthouserc.json</code> at the repo root.
        </p>
        <dl className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <Stat label="Performance" value="≥ 90" />
          </div>
          <div>
            <Stat label="Accessibility" value="≥ 95" />
          </div>
          <div>
            <Stat label="Best Practices" value="≥ 90" />
          </div>
          <div>
            <Stat label="SEO" value="≥ 95" />
          </div>
        </dl>
        <p className="mt-3 text-xs text-neutral-500">
          Reports upload to temporary public storage; the URL is in the GitHub
